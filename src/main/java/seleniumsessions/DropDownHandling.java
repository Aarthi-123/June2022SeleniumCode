...
//		doSelectDropDownByValue(country, "Brazil");
		...
		ArrayList<String> countryDropDownOptionsTextList = getDropDownOptionsTextList(country);
		System.out.println(countryDropDownOptionsTextList);
	}
	...
	public static ArrayList<String> getDropDownOptionsTextList(By locator){
		List<WebElement> countryOptions = getDropDownOptions(locator);
		ArrayList<String> countryOptionsList = new ArrayList<String>();
		for(WebElement e:countryOptions) {
			String text = e.getText();
			System.out.println(text);
			if(text.length()!=0) {
